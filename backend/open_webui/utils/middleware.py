...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if tool_servers:
        for model_tool in tool_servers:
            tool_specs = model_tool.pop("specs", [])
    ...
    if not isinstance(response, StreamingResponse):
        ...
        if has_custom_tools() and isinstance(response, dict):
            ...
            if choices and choices[0].get("message", {}).get("tool_calls"):
                ...
                if tool_results:
                    current_messages = form_data.get("messages", []) + [
                        choices[0]["message"]  # The assistant message with tool calls
                    ...
                    while iteration < max_iterations:
                        ...
                        follow_up_response = await generate_chat_completion(
                            request,
                            {
                                ...
                                **{k: v for k, v in form_data.items() 
                                   if k not in ["messages", "stream", "tools"] and v is not None}
                            },
                            user
                        )
                        
                        # Check if this response has more tool calls
                        if (isinstance(follow_up_response, dict) and 
                            follow_up_response.get("choices", []) and 
                            follow_up_response["choices"][0].get("message", {}).get("tool_calls")):
                            
                            # Process the additional tool calls using the same helper function
                            additional_tool_calls = follow_up_response["choices"][0]["message"]["tool_calls"]
                            additional_tool_results = await process_tool_calls(additional_tool_calls, tools_dict)
                        ...
                        response = follow_up_response
                        log.warning(f"Max tool call iterations ({max_iterations}) reached for custom model tools")
